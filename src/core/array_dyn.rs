...
impl<T> IntoIterator for DynArray<T> {
    type Item = T;
    type IntoIter =
        <<ArrayBase<T, 0, true> as ArrayStorage<T>>::StorageVariant as NdArrayBase>::IntoIter;
...
impl<'a, T> IntoIterator for &'a DynArray<T> {
    type Item = &'a T;
    type IntoIter =
        <<ArrayBase<T, 0, true> as ArrayStorage<T>>::StorageVariant as NdArrayBase>::Iter<'a>;
...
impl<'a, T> IntoIterator for &'a mut DynArray<T> {
    type Item = &'a mut T;
    type IntoIter =
        <<ArrayBase<T, 0, true> as ArrayStorage<T>>::StorageVariant as NdArrayBase>::IterMut<'a>;
...
mod tests {
    ...
    fn dyn_array_scalar_type() {
        assert_eq!(
            type_name::<<DynArray<u32> as NdArray>::Scalar>(),
            type_name::<<DynArray<Array<u32, 4>> as NdArray>::Scalar>()
        );
        assert_ne!(
            type_name::<<DynArray<Array<f32, 4>> as NdArray>::Scalar>(),
            type_name::<<DynArray<Array<u32, 4>> as NdArray>::Scalar>()
        );
    }
}
